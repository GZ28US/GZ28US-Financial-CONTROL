-- VOLTA ATRÁS DO BACKFILL_shopping_invoice_elos_BR.sql (14/set/2026).
...
create temp table volta on commit drop as
  select f.row_id, f.field, f.new_value
    from public.data_fixes f where f.check_key = 'shopping-invoice-elos' and f.table_name = 'invoice_expenses'
     and f.field in ('us_expense_id', 'mirror_src') and f.old_value is null;
...
select 'shopping-invoice-elos-volta', 'invoice_expenses', e.id::text, v.field, v.new_value, null, 'volta dos elos de 14/set'
 ...
 where (v.field = 'us_expense_id' and e.us_expense_id::text = v.new_value) or (v.field = 'mirror_src' and e.mirror_src = v.new_value);

update public.invoice_expenses e set us_expense_id = null from volta v where v.field = 'us_expense_id' and e.id::text = v.row_id and e.us_expense_id::text = v.new_value;
...
select count(*) filter (where us_expense_id is not null) com_us_expense_id, count(*) filter (where mirror_src is not null) com_mirror_src from public.invoice_expenses;
