...
const COLUNAS_NOVAS: Record<Banco, [string, string][]> = {
  US: [['invoices', 'mirror_key'], ['invoice_expenses', 'mirror_src'], ['invoice_items', 'mirror_src'], ['invoice_incomes', 'mirror_src'], ['invoice_incomes', 'br_payment_id']],
...
export type Op =
  ...
  | { tipo: 'carimbo'; banco: Banco; tabela: string; id: string; campo: string; valor: number; regra: string; direcao: Direcao; rotulo: string }
  | { tipo: 'criar_linha'; banco: Banco; tabela: string; mirror_src: string; campos: Row; usd: number; brl: number | null; direcao: Direcao; papel: 'despesa' | 'item' | 'renda'; rotulo: string }
  ...
  | { tipo: 'pendente_apagar'; banco: Banco; tabela: string; id: string; de: number; rotulo: string }
...
function planejarUS(foto: Foto, cot: Cotacoes, excluidos: Excluido[], correcoes: CorrecaoB[]): ChavePlano[] {
  ...
  for (const brId of idsBR) {
    ...
    for (const p of pares) {
      ...
      if (!it) m.conflito(`a despesa ${limpa(p.e.item).slice(0, 40)} não tem item correspondente na ${uInv.get(p.e.invoice_id)?.invoice_code}`)
    ...
    for (const s of sobraS) {
      ...
      m.op({ tipo: 'criar_linha', banco: 'US', tabela: 'invoice_expenses', mirror_src: src, direcao: 1, papel: 'despesa', usd: C, brl: r2(brlBR(s)), rotulo: `despesa ${limpa(s.item).slice(0, 60)}`, campos: {
      ...
      m.op({ tipo: 'criar_linha', banco: 'US', tabela: 'invoice_items', mirror_src: src, direcao: 1, papel: 'item', usd: r2(Rv), brl: null, rotulo: `item ${limpa(s.item).slice(0, 60)}`, campos: item })
    }
    ...
    for (const p of P2) {
      ...
      if (ja) {
        ...
        m.op({ tipo: 'criar_linha', banco: 'US', tabela: 'invoice_incomes', mirror_src: src, direcao: 2, papel: 'renda', usd: usd as number, brl: num(p.amount), rotulo: `renda ${String(p.description || '').slice(0, 60)}`, campos })
      }
    ...
    const devido = r2(grand - recebido)
    ...
    if (temEscrita && !B.us_invoice_id) m.op({ tipo: 'ponteiro_origem', banco: 'BR', tabela: 'invoices', id: B.id, campo: 'us_invoice_id', rotulo: `${B.invoice_code}.us_invoice_id → a 006.N` })
...
function planejarBR(foto: Foto, cot: Cotacoes, excluidos: Excluido[]): ChavePlano[] {
  ...
  for (const [key, g] of grupos) {
    ...
    for (const p of pares) {
      ...
      if (!pt) m.conflito(`a despesa ${limpa(p.e.item).slice(0, 40)} não tem item correspondente na ${bInv.get(p.e.invoice_id)?.invoice_code}`)
    ...
    for (const f of [...sobraF].sort((x, y) => x.dia.localeCompare(y.dia) || x.item.localeCompare(y.item))) {
      ...
      const src = srcDe(f)
      m.op({ tipo: 'criar_linha', banco: 'BR', tabela: 'invoice_expenses', mirror_src: src, direcao: 3, papel: 'despesa', usd: lineUsd, brl: lineBrl, rotulo: `despesa ${f.item.slice(0, 50)} · ${regra}`, campos: {
      ...
      m.op({ tipo: 'criar_linha', banco: 'BR', tabela: 'invoice_parts', mirror_src: src, direcao: 3, papel: 'item', usd: lineUsd, brl: lineBrl, rotulo: `item ${f.item.slice(0, 50)}`, campos: parte })
    }
    ...
    for (const s of g.rendas) {
      ...
      const jaElo = PgPagos.find(p => p.mirror_src === src || p.us_income_id === s.id)
      ...
      if (ja) {
        ...
        m.op({ tipo: 'criar_linha', banco: 'BR', tabela: 'invoice_payments', mirror_src: src, direcao: 4, papel: 'renda', usd, brl, rotulo: `renda ${String(s.description || '').slice(0, 50)} · ${regra}`, campos })
      }
    }
    ...
    if (temEscrita && g.tipo === 'invoice' && U && !U.br_invoice_id) m.op({ tipo: 'ponteiro_origem', banco: 'US', tabela: 'invoices', id: U.id, campo: 'br_invoice_id', rotulo: `${U.invoice_code}.br_invoice_id → a 085.N` })
  }
...
// A ESCRITA
// ════════════════════════════════════════════════════════════════════════════
export type ResultadoChave = { mirror_key: string; resultado: 'aplicada' | 'pulada' | 'recusada' | 'erro'; motivo: string | null; codigo: string | null; escritas: number }
export type ResultadoAplicacao = { ok: boolean; chaves: ResultadoChave[]; parou_em: string | null }

/**
 * Grava o plano. Exige confirm:true. Refaz o plano AGORA e só grava a chave cuja impressão digital
 * bate com a do plano recebido (e com `impressoes`, se vier) — plano velho não escreve. Conflito e
 * «nada» são pulados. A primeira escrita que não confere para tudo (falha fechada); como tudo é
 * achado por mirror_key/mirror_src, rodar de novo continua de onde parou.
 */
export async function applyPlan(b: Bancos, plano: Plano, opcoes: { confirm: boolean; keys?: string[]; impressoes?: Record<string, string> }): Promise<ResultadoAplicacao> {
  ...
  if (!fresco.migrado.US || !fresco.migrado.BR) throw new ErroTravessia('schema', `As migrations da travessia não rodaram — faltam: US [${fresco.migrado.faltando.US.join(', ')}] · BR [${fresco.migrado.faltando.BR.join(', ')}]. Nada foi escrito.`)
  const escolhidas = opcoes.keys?.length ? new Set(opcoes.keys) : null
  const out: ResultadoChave[] = []
  for (const chave of plano.chaves) {
    if (escolhidas && !escolhidas.has(chave.mirror_key)) continue
    ...
    if (f.impressao !== chave.impressao || (opcoes.impressoes && opcoes.impressoes[chave.mirror_key] && opcoes.impressoes[chave.mirror_key] !== f.impressao)) { out.push({ ...base, resultado: 'recusada', motivo: 'o plano mudou desde a leitura — rode o GET de novo e confira' }); continue }
    if (f.status === 'conflito' || f.status === 'nada') { out.push({ ...base, resultado: 'pulada', motivo: f.status === 'conflito' ? f.conflitos.join(' · ') : 'nada a fazer' }); continue }
    const r = await executarChave(b, f)
    out.push({ ...base, ...r })
  ...
  try {
    for (const o of c.ops) {
      ...
      if (o.tipo === 'vincular_invoice') {
        ...
        if (error) throw new ErroTravessia('db', `vincular ${o.codigo}: ${error.message}`)
        if (!data?.length) {
          const { data: r, error: e2 } = await db.from('invoices').select('mirror_key').eq('id', o.id).maybeSingle()
          if (e2 || r?.mirror_key !== c.mirror_key) throw new ErroTravessia('conflict', `${o.codigo} não aceitou a mirror_key (já tem outra, ou sumiu)`)
        ...
        if (error || !ins) throw new ErroTravessia('db', `criar ${cod}: ${error?.message || 'sem linha'}`)
        ...
        if (error) throw new ErroTravessia('db', `ponteiro ${o.campo}: ${error.message}`)
        if (!data?.length) {
          const { data: r } = await db.from(o.tabela).select(o.campo).eq('id', o.id).maybeSingle()
        ...
        if (error) throw new ErroTravessia('db', `vincular ${o.tabela} ${o.id}: ${error.message}`)
        if (!data?.length) {
          const { data: r } = await db.from(o.tabela).select('mirror_src').eq('id', o.id).maybeSingle()
        ...
        const { data, error } = await db.from(o.tabela).update({ [o.campo]: o.valor }).eq('id', o.id).is(o.campo, null).select('id')
        if (error) throw new ErroTravessia('db', `carimbo ${o.tabela}.${o.campo}: ${error.message}`)
        if (!data?.length) {
          const { data: r } = await db.from(o.tabela).select(o.campo).eq('id', o.id).maybeSingle()
          if (!perto(num((r as any)?.[o.campo]), o.valor, 0.005)) throw new ErroTravessia('conflict', `${o.tabela}.${o.campo} ${o.id} já tem outro valor gravado — o gravado prevalece, o plano não serve mais`)
        ...
        if (error || !ins) throw new ErroTravessia('db', `criar ${o.tabela} (${o.mirror_src}): ${error?.message || 'sem linha'}`)
        escritas++
        await trilha(o.banco, o.tabela, ins.id, '(linha criada)', null, o.mirror_src, o.rotulo)
      } else if (o.tipo === 'pendente_atualizar') {
        const { data, error } = await db.from(o.tabela).update({ amount: o.para, mirror_src: o.mirror_src }).eq('id', o.id).is('paid_at', null).eq('amount', o.de).select('id')
        if (error) throw new ErroTravessia('db', `pendente ${o.id}: ${error.message}`)
        if (data?.length !== 1) throw new ErroTravessia('conflict', `o Pending balance ${o.id} mudou (baixa ou valor) antes da escrita`)
        escritas++
        ...
        if (eAntes || !antes) throw new ErroTravessia('db', `ler o Pending balance ${o.id} antes de apagar: ${eAntes?.message || 'sumiu'}`)
        await trilha(o.banco, o.tabela, o.id, '(pendente apagado)', JSON.stringify(antes), null, o.rotulo)
        const { data, error } = await db.from(o.tabela).delete().eq('id', o.id).is('paid_at', null).eq('amount', o.de).select('id')
        if (error) throw new ErroTravessia('db', `apagar pendente ${o.id}: ${error.message}`)
        if (data?.length !== 1) throw new ErroTravessia('conflict', `o Pending balance ${o.id} mudou (baixa ou valor) antes de sair — a trilha registrou a tentativa, nada saiu`)
        escritas++
      }
    }
    return { resultado: 'aplicada', motivo: null, codigo, escritas }
  } catch (e) {
    return { resultado: 'erro', motivo: e instanceof Error ? e.message : String(e), codigo, escritas }
  }
}
