...
export async function runStaffTravelSweep(db: SupabaseClient): Promise<{ opened: string[]; closed: string[] }> {
  ...
  for (const m of inbox?.value || []) {
    ...
    if (opening) {
      ...
      if (se) {
        // Linha NOVA de staff: PAID TO nasce GZ28US, escondido (lib/payerRule). Quem pagou a passagem
        // é escolha — o e-mail da cia aérea não diz — e segue pergunta do Data Checker.
        await db.from('staff_expenses').insert({ season_id: se.id, type: 'SINGLE', amount: p.total, expense_date: today, payment_date: today, description: `Passagem de IDA — ${p.pnr} (${p.last}/${p.first}) ${origin}-${dest}`, source: 'Auto-captura e-mail (staff travel)', ...hiddenPayers('staff_expenses', null, true) })
      ...
      const season = open?.[0]
      if (season) {
        await db.from('staff_expenses').insert({ season_id: season.id, type: 'SINGLE', amount: p.total, expense_date: today, payment_date: today, description: `Passagem de VOLTA — ${p.pnr} (${p.last}/${p.first}) ${origin}-${dest}`, source: 'Auto-captura e-mail (staff travel)', ...hiddenPayers('staff_expenses', null, true) })
