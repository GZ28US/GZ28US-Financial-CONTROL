...
const MIGRATION_RE = /match_engine|match_batch|reviewed_at|backfill|bank_transaction_id|match_rule|bank_auto_runs|pfc_|klass|priority|invoices_bucket|bank_merchant_rules_key|doubt_answered|matched_members/
...
// CASAR COM AJUSTE: quem é a season (nome do staff), quais registros da folha já estão
// tomados por ponteiro (EXACT/NAME/humano casam por matched_id, sem elo na coluna), e a
// memória de recusa da linha (NÃO É ESSE).
async function staffNames(db: any): Promise<Map<string, string>> {
  const [seasons, staff] = await Promise.all([fetchAll(db, 'seasons', 'id, staff_id'), fetchAll(db, 'staff', 'id, name')])
  const nameOf = new Map<string, string>(staff.map((x: any) => [String(x.id), String(x.name || '')]))
  return new Map<string, string>(seasons.map((s: any) => [String(s.id), nameOf.get(String(s.staff_id)) || '?']))
}
// Membro da folha num casamento MISTO vivo também está tomado (BL 1.6.0): o misto restaurado sem elo, ou com o DESFAZER falho no
// meio, não pode virar candidato do CASAR COM AJUSTE de outra linha — contaria a passagem duas vezes e reescreveria o valor dela.
async function expenseTaken(db: any): Promise<Set<string>> {
  const rows = await fetchBankLines(db, 'matched_table, matched_id', (q: any) => q.in('matched_table', ['staff_expenses', MIXED_GROUP]).neq('match_status', 'REMOVED').not('matched_id', 'is', null))
  const out = new Set<string>()
  for (const r of rows) { if (r.matched_table === 'staff_expenses') out.add(String(r.matched_id)); else for (const m of mixedMembers(r)) if (m.table === 'staff_expenses') out.add(m.id) }
  return out
}
...
// Folha desde 15 dias antes da abertura da Regions (a régua é ±10 d da LINHA do banco, não de hoje): o backlog de nov/2025 também tem passagem.
const since150 = () => '2025-10-26'
const usdTol = (amt: number) => '$' + adjustTol(amt).toFixed(0)
// Rateio do Δ ao centavo: proporcional, resto no maior (empate: menor id) — determinístico.
function allocate(bank: number, rows: { id: string; amount: number }[]): Map<string, number> {
  const sum = rows.reduce((s, r) => s + r.amount, 0)
  const out = new Map<string, number>()
  let acc = 0
  for (const r of rows) { const v = Math.round(bank * (sum ? r.amount / sum : 1 / rows.length) * 100) / 100; out.set(r.id, v); acc += v }
...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    if (req.nextUrl.searchParams.get('autobook') === '1') {
      const runsQ = await db.from('bank_auto_runs').select('*').order('started_at', { ascending: false }).limit(200)
      ...
      // ADJUST é gente (CASAR COM AJUSTE), não «sozinho».
      const by = (since: string) => logs.filter((r: any) => r.at >= since && r.engine !== 'ADJUST').reduce((m: Record<string, number>, r: any) => { m[r.engine] = (m[r.engine] || 0) + 1; return m }, {})
      ...
      try {
        ...
        const blines = await fetchBankLines(db, 'id, name, merchant, amount, match_status, matched_table, matched_id, reviewed_at, match_engine', (q: any) => q.eq('match_status', 'MATCHED').in('matched_table', ['invoice_expenses', 'purchase_group', 'staff_expenses', 'expense_group', MIXED_GROUP]))
        ...
        // Membro de casamento MISTO vivo tem dono (BL 1.6.0): não é ÓRFÃO, e o valor dele é só parte da linha (o VALOR MUDOU do misto é conferido abaixo, pela soma).
        for (const l of blines) { if (l.matched_table === 'invoice_expenses') byRow.set(String(l.matched_id), l); else if (l.matched_table === 'purchase_group') byGroup.set(String(l.matched_id), l); else if (l.matched_table === MIXED_GROUP) { for (const m of mixedMembers(l)) if (m.table === 'invoice_expenses' && !byRow.has(m.id)) byRow.set(m.id, l) } }
        for (const r of brows) {
          ...
          if (l.matched_table !== MIXED_GROUP && Math.abs(expLine(r) - Math.abs(num(l.amount))) >= 0.011) amountDrift.push({ bank_id: l.id, row_id: r.id, bank_amount: Math.abs(num(l.amount)), row_amount: expLine(r), label: [r.supplier, r.item].filter(Boolean).join(' · ') })
          // dupla do balde: gêmeo humano em QUALQUER tabela, valor na faixa do imposto, ±14d, nome batendo
          if (!l.reviewed_at && r.payment_date && l.matched_table !== MIXED_GROUP) {
        ...
        // ELO SOLTO (CASAR COM AJUSTE): passagem da folha ligada a uma linha do banco que não a aponta mais.
        try {
          const linkedRows = await expensesRows(db, 'id, description, amount', (q: any) => q.not('bank_transaction_id', 'is', null))
          const lids = [...new Set(linkedRows.map((r: any) => String(r.bank_transaction_id)))]
          const lptr = new Map<string, any>()
          for (let i = 0; i < lids.length; i += 200) { const chunk = lids.slice(i, i + 200); for (const x of await fetchBankLines(db, 'id, match_status, matched_table, matched_id', (q: any) => q.in('id', chunk))) lptr.set(String(x.id), x) }
          for (const r of linkedRows) {
            const x = lptr.get(String(r.bank_transaction_id))
            // Casamento MISTO (BL 1.6.0): o elo vale enquanto a passagem é MEMBRO gravado da linha.
            const ok = x && x.match_status === 'MATCHED' && ((x.matched_table === 'staff_expenses' && String(x.matched_id) === String(r.id)) || (x.matched_table === 'expense_group' && String(x.matched_id) === String(x.id)) || (x.matched_table === MIXED_GROUP && String(x.matched_id) === String(x.id) && mixedMembers(x).some(m => m.table === 'staff_expenses' && m.id === String(r.id))))
        ...
        for (const l of blines) {
          if (l.match_engine !== ENGINE_BUCKET && !['staff_expenses', 'expense_group', MIXED_GROUP].includes(String(l.matched_table))) continue
          if (l.matched_table === MIXED_GROUP) {
            // CASAMENTO MISTO (BL 1.6.0): membro que sumiu = ponteiro morto (DESFAZER devolve a linha); membros cujo líquido dá outro valor = VALOR MUDOU.
            const ms = mixedMembers(l)
            const amts = await memberAmounts(db, ms)
            const gone = ms.filter(m => !amts.has(m.table + ':' + m.id))
            if (!ms.length || gone.length) deadPointers.push({ bank_id: l.id, table: MIXED_GROUP, id: l.matched_id, label: (l.merchant || l.name || '') + ' · misto: ' + (ms.length ? gone.map(m => m.table).join(', ') + ' sumiu' : 'sem membros gravados'), amount: Math.abs(num(l.amount)) })
            // A soma é COM SINAL (BL 1.6.1, mixedNet — a régua do match_mixed): o registro do lado oposto da linha desconta (o repasse
            // da Stripe = créditos das credenciais − a tarifa). Soma crua em valor absoluto chamava o casamento líquido certo de VALOR MUDOU.
            else { const mn = mixedNet(num(l.amount), ms.map(m => ({ table: m.table, id: m.id, amount: amts.get(m.table + ':' + m.id) || 0 }))); if (!mn.ok) amountDrift.push({ bank_id: l.id, row_id: ms[0].id, bank_amount: Math.abs(num(l.amount)), row_amount: mn.net, label: (l.merchant || l.name || '') + ' · misto ×' + ms.length }) }
            continue
          }
    ...
    if (req.nextUrl.searchParams.get('bucket') === '1') {
      ...
      // CASAMENTO MISTO (BL 1.6.0): linha do balde que é membro de misto vivo tem dono (casamento de gente, sem motor) — não é ÓRFÃ.
      { const inBucket = new Set(rowIds); for (const l of await fetchBankLines(db, BSEL, (q: any) => q.eq('match_status', 'MATCHED').eq('matched_table', MIXED_GROUP))) for (const m of mixedMembers(l)) if (m.table === 'invoice_expenses' && inBucket.has(m.id) && !byRow.has(m.id)) byRow.set(m.id, l) }
      ...
      const rowsOut = brows.map((r: any) => {
        ...
        return {
          ...
          orphan: !l, suggestions: suggest(r, l && l.matched_table === MIXED_GROUP ? null : l),
    ...
    if (req.nextUrl.searchParams.get('questions') === '1') {
      ...
      try {
        const [expRows, staffOf, takenExp] = await Promise.all([expensesRows(db, undefined, (q: any) => q.gte('expense_date', since150())), staffNames(db), expenseTaken(db)])
        const expFree = expRows.filter((r: any) => !takenExp.has(String(r.id)))
      ...
      const today = todayNY()
      return NextResponse.json({ ok: true, suppliers, money, twins: twinsQ, link_migration: expenseLinkColumnMissing(), caps: of('CAP').length, maturity: of('MATURITY').length, other: of('OTHER').length, seasons, categories: INPUT_CATEGORIES,
    ...
    try {
      const doubtIds = Object.keys(planNow.doubts)
      if (doubtIds.length) {
        const [expRows, staffOf, takenExp] = await Promise.all([expensesRows(db, undefined, (q: any) => q.gte('expense_date', since150())), staffNames(db), expenseTaken(db)])
        const expFree = expRows.filter((r: any) => !takenExp.has(String(r.id)))
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    if (action === 'match_adjust') {
      ...
      const ids = [...new Set((Array.isArray(body.ids) ? body.ids : []).map(String))].slice(0, 3)
      if (!bankId2 || !ids.length) return NextResponse.json({ error: 'bank_id/ids required' }, { status: 400 })
      const { data: line0, error: lErr } = await db.from('bank_transactions').select(BSEL + ', doubt_answered').eq('id', bankId2).maybeSingle()
      const line: any = line0
      if (lErr) return NextResponse.json({ error: lErr.message, needs_migration: MIGRATION_RE.test(lErr.message) }, { status: 500 })
      if (!line || !['NEW', 'QUEUED'].includes(String(line.match_status))) return NextResponse.json({ error: 'linha do banco já decidida — recarregue' }, { status: 409 })
      if (line.pending) return NextResponse.json({ error: 'linha ainda PENDING no banco — espere postar' }, { status: 409 })
      if (!(num(line.amount) > 0)) return NextResponse.json({ error: 'só saída casa com passagem' }, { status: 409 })
      const bank = Math.round(Math.abs(num(line.amount)) * 100) / 100
      const [rows0, staffOf, takenExp] = await Promise.all([expensesRows(db, undefined, (q: any) => q.in('id', ids)), staffNames(db), expenseTaken(db)])
      if (rows0.length !== ids.length) return NextResponse.json({ error: 'registro da folha não existe mais — recarregue' }, { status: 409 })
      if (rows0.some((r: any) => r.bank_transaction_id || String(r.payment_reference || '').startsWith('bank:') || takenExp.has(String(r.id)))) return NextResponse.json({ error: 'registro da folha já casado com outra linha — recarregue' }, { status: 409 })
      // A mesma régua da tela (nome, datas, tolerância): o pedido tem que ser um candidato válido.
      const cand = nearExpenseMatches(line, rows0, staffOf, rejectedOf(line)).find(c => c.ids.length === ids.length && c.ids.every(i => ids.includes(i)))
      if (!cand) return NextResponse.json({ error: 'candidato não vale: nome, datas ou valor fora da régua (tolerância ' + usdTol(bank) + ')' }, { status: 409 })
      ...
      // Leitura que falha depois das escritas devolve tudo (revisão da BL 1.6.0): senão o elo e o valor ficavam reescritos com a linha NEW.
      let takenNow: Set<string>
      try { takenNow = await expenseTaken(db) } catch (e) { await rollback(); return NextResponse.json({ error: String((e as Error).message || e).slice(0, 200) }, { status: 500 }) }
      ...
      await db.from('data_fixes').insert(fixes).then(() => undefined, () => undefined)
      return NextResponse.json({ ok: true, engine: 'ADJUST', delta: cand.delta, rows: rows0.length, note })
    }
    // ── CASAMENTO MISTO (BL 1.6.0 — Márcio, 13/set/2026): UMA linha do banco ⇄ registros de TABELAS DIFERENTES ──
    // O cupom do posto que cobrou cigarro PESSOAL e cerveja de insumo juntos (Wawa 5205, 10/set: $115,44 = PESSOAL $90,15 + insumo $25,29).
    ...
    // MISTO COM SINAL (BL 1.6.1, 13/set): o repasse da Stripe de 11/ago, −$1.318,84 = 4 vendas de credencial SEMA (créditos em
    // fixed_cost_expenses, +$1.375,00) − a tarifa da Stripe ($56,16) — registro do lado oposto da linha DESCONTA (mixedNet).
    if (action === 'match_mixed') {
      const bankId2 = String(body.bank_id || '')
      const raw: any[] | null = Array.isArray(body.members) ? body.members : null
      if (!bankId2 || !raw) return NextResponse.json({ error: 'bank_id/members required' }, { status: 400 })
      const { data: line0, error: lErr } = await db.from('bank_transactions').select(BSEL + ', matched_members').eq('id', bankId2).maybeSingle()
      const line: any = line0   // select composto: o tipo do supabase-js não resolve a string concatenada (mesmo truque do match_adjust)
      if (lErr) return NextResponse.json({ error: lErr.message, needs_migration: MIGRATION_RE.test(lErr.message) }, { status: 500 })
      ...
      if (line.pending) return NextResponse.json({ error: 'linha ainda PENDING no banco — espere postar' }, { status: 409 })
      // A DIREÇÃO da linha decide o SINAL de cada membro, não a tabela (BL 1.6.1, misto com sinal — o repasse da Stripe, 13/set; na
      // BL 1.6.0 a saída só aceitava MIXED_TABLES do pool.out e a entrada só rendas do pool.inn). Tabela: qualquer uma de mixable()
      // (as sete de gasto e a renda) — nada de aporte, empréstimo, pedido ou invoice_items. Cada membro é procurado nos DOIS pools:
      // do lado da linha soma, do lado oposto desconta (mixedNet, a régua única): o crédito de gasto (valor negativo, pool.inn) entra
      // numa linha que ENTROU, a despesa comum (pool.out) desconta numa entrada, a renda desconta numa saída.
      ...
      const badT = [...new Set(members.filter(m => !mixable(m.table)).map(m => m.table))]
      if (badT.length) return NextResponse.json({ error: 'tabela fora do casamento misto: ' + badT.join(', ') + ' (vale ' + [...MIXED_TABLES, ...MIXED_IN_TABLES].join(', ') + ')' }, { status: 400 })
      // Cada membro tem de ser candidato LIVRE do pool AGORA, em um dos dois lados: livre de linha viva (inclusive como membro de outro
      // misto), não paga pela GZ28BR (renda: não caída na GZ28BR), não espelho de despesa, não datada no futuro, valor não zero, estoque
      // só PURCHASED, invoice só real (não orçamento). O valor sai do pool (absoluto) — nunca do pedido.
      const pool = await candidatePool(db)
      const found = mixedFromPool(pool, amt0, members)
      if (found.missing.length) return NextResponse.json({ error: 'registro não é candidato livre (já casado — inclusive como membro de outro misto —, pago pela GZ28BR ou renda caída na GZ28BR, espelho de despesa, datado no futuro, valor zero, estoque não comprado ou invoice orçamento) — recarregue: ' + found.missing.join(', ') }, { status: 409 })
      if (found.both.length) return NextResponse.json({ error: 'registro aparece nos dois lados do pool (entrada e saída) — recarregue: ' + found.both.join(', ') }, { status: 409 })
      const cands = found.cands, mn = found.net
      ...
      const staffIds = members.filter(m => m.table === 'staff_expenses').map(m => m.id)
      let staffRows: any[] = []
      if (staffIds.length) {
        if (await probeExpenseLink(db)) return NextResponse.json({ error: 'rode MIGRATION_expenses_bank_link.sql antes (coluna expenses.bank_transaction_id)', needs_link_migration: true }, { status: 409 })
        staffRows = await expensesRows(db, 'id, description, amount, payment_reference', (q: any) => q.in('id', staffIds))
        if (staffRows.length !== staffIds.length) return NextResponse.json({ error: 'registro da folha não existe mais — recarregue' }, { status: 409 })
        const withLink = staffRows.filter((r: any) => r.bank_transaction_id)
        if (withLink.length) return NextResponse.json({ error: 'registro da folha já tem elo com uma linha do banco (linha morta: SOLTAR no Data Checker) — recarregue: ' + withLink.map((r: any) => 'staff_expenses:' + r.id).join(', ') }, { status: 409 })
        const refs = [...new Set(staffRows.map((r: any) => String(r.payment_reference || '').startsWith('bank:') ? String(r.payment_reference).slice(5) : '').filter(Boolean))]
        const refLines = refs.length ? await fetchBankLines(db, 'id, match_status, matched_id', (q: any) => q.in('id', refs)) : []
        const liveRef = new Set(refLines.filter((x: any) => x.match_status !== 'REMOVED' && x.matched_id).map((x: any) => String(x.id)))   // a régua do linkedLines do pool
        const linked = staffRows.filter((r: any) => String(r.payment_reference || '').startsWith('bank:') && liveRef.has(String(r.payment_reference).slice(5)))
        if (linked.length) return NextResponse.json({ error: 'registro da folha já ligado a outra linha do banco — recarregue: ' + linked.map((r: any) => 'staff_expenses:' + r.id).join(', ') }, { status: 409 })
      }
      // O elo da folha ANTES do claim (a mesma ordem do CASAR COM AJUSTE), guardado pelo elo VAZIO; entra no backfill como `pre`.
      const pre: Backfill[] = []
      const claimedStaff: any[] = []
      const rollback = async () => { for (const r of claimedStaff) await db.from('staff_expenses').update({ bank_transaction_id: null }).eq('id', r.id).eq('bank_transaction_id', line.id) }
      for (const r of staffRows.slice().sort((a: any, b: any) => String(a.id).localeCompare(String(b.id)))) {
        const { data: ok, error } = await db.from('staff_expenses').update({ bank_transaction_id: line.id }).eq('id', r.id).is('bank_transaction_id', null).select('id')
        if (error) { await rollback(); return NextResponse.json({ error: 'staff_expenses: ' + error.message }, { status: 500 }) }
        if (!ok || !ok.length) { await rollback(); return NextResponse.json({ error: 'registro da folha mudou enquanto você olhava — recarregue' }, { status: 409 }) }
        claimedStaff.push(r)
        pre.push({ t: 'staff_expenses', id: String(r.id), f: 'bank_transaction_id', v: String(line.id), o: null })
      }
      // Recheca o ponteiro da folha DEPOIS do elo (EXACT/NAME/humano casam por matched_id, sem elo na coluna).
      // Leitura que falha aqui devolve o elo (revisão da BL 1.6.0): nunca fica elo escrito com a linha NEW.
      if (staffIds.length) {
        let takenNow: Set<string>
        try { takenNow = await expenseTaken(db) } catch (e) { await rollback(); return NextResponse.json({ error: String((e as Error).message || e).slice(0, 200) }, { status: 500 }) }
        if (staffIds.some(i => takenNow.has(i))) { await rollback(); return NextResponse.json({ error: 'registro da folha acabou de ser casado por outra linha — recarregue' }, { status: 409 }) }
      }
      // Nota com o LÍQUIDO na frente (BL 1.6.1): «MISTO · 5 registros · +1375,00 −56,16 · …» — o que soma e o que desconta, depois os registros.
      const brl = (v: number) => v.toFixed(2).replace('.', ',')
      const short = (c: any, sign: number) => { const seg = String(c.label || '').split(' · '); return (sign < 0 ? '−' : '+') + seg[0] + (seg[1] ? ' ' + seg[1].slice(0, 22) : '') + ' $' + num(c.amount).toFixed(2) }
      const note = (String(body.note || '').trim() || ('MISTO · ' + members.length + ' registros · +' + brl(mn.plus) + (mn.minus ? ' −' + brl(mn.minus) : '') + ' · ' + cands.map((c: any, i: number) => short(c, mn.parts[i].sign)).join(' '))).slice(0, 150)
      const cnd = { table: MIXED_GROUP, id: String(line.id), members }
      try { await writeMatch(db, line, cnd, { matched_members: members, matched_note: note, match_engine: null, match_batch: null, match_rule: null, reviewed_at: new Date().toISOString() }, pre) }
      catch (e) {
        // Só devolve o elo se o casamento NÃO pegou (relê a linha); releitura que falha não prova nada — nada é revertido às cegas.
        const { data: now0, error: rErr } = await db.from('bank_transactions').select('match_status, matched_table, matched_id, matched_members').eq('id', line.id).maybeSingle()
        if (rErr) return NextResponse.json({ error: 'confira no Bank Link: o casamento misto falhou no meio e a linha não pôde ser relida — ' + String((e as Error).message || e).slice(0, 120) }, { status: 500 })
        // Pegou só se a linha guarda ESTES membros: outro misto na mesma linha (outra aba, reenvio com lista corrigida) não é o nosso
        // — aí o elo que ESTE pedido escreveu volta, senão ficava ELO SOLTO preso numa linha que não o lista.
        const mine = new Set(keys), got = mixedMembers(now0).map(m => m.table + ':' + m.id)
        const landed = !!now0 && now0.match_status === 'MATCHED' && now0.matched_table === MIXED_GROUP && String(now0.matched_id) === String(line.id) && got.length === mine.size && got.every(k => mine.has(k))
        if (!landed) await rollback()
        const msg = String((e as Error).message || e)
        return NextResponse.json({ error: msg.slice(0, 200), needs_migration: MIGRATION_RE.test(msg) }, { status: 409 })
      }
      const parts = mn.parts.map((p, i) => (i ? (p.sign > 0 ? ' + ' : ' − ') : (p.sign > 0 ? '' : '−')) + p.table + ' $' + p.amount.toFixed(2)).join('')
      await db.from('data_fixes').insert({ check_key: 'bank-reconcile', table_name: 'bank_transactions', row_id: line.id, field: 'match_status', old_value: String(line.match_status), new_value: 'MATCHED', label: ('MISTO · ' + line.date + ' · ' + (line.merchant || line.name || '') + ' · $' + bank.toFixed(2) + ' = ' + parts).slice(0, 200) }).then(() => undefined, () => undefined)
      // `sum` é o LÍQUIDO (plus − minus); cada membro diz o sinal com que entrou na conta (+1 do lado da linha, −1 do lado oposto).
      return NextResponse.json({ ok: true, status: 'MATCHED', bank, sum: mn.net, plus: mn.plus, minus: mn.minus, note, members: cands.map((c: any, i: number) => ({ table: c.table, id: c.id, amount: num(c.amount), sign: mn.parts[i].sign, label: c.label })) })
    }
    // SOLTAR (Data Checker · ELO SOLTO): a passagem diz que casou, a linha do banco não a aponta — só limpa o elo.
    if (action === 'unlink_expense') {
      ...
      const mixedOwner = l0 && l0.matched_table === MIXED_GROUP ? (await fetchBankLines(db, 'id, matched_table', (q: any) => q.eq('id', l0.id)))[0] : null   // os membros moram na coluna nova
      if (l0 && l0.match_status === 'MATCHED' && ((l0.matched_table === 'staff_expenses' && String(l0.matched_id) === rowId) || (l0.matched_table === 'expense_group' && String(l0.matched_id) === String(l0.id)) || (l0.matched_table === MIXED_GROUP && mixedMembers(mixedOwner).some(m => m.table === 'staff_expenses' && m.id === rowId)))) return NextResponse.json({ error: 'a linha do banco ainda aponta pra este registro — DESFAZER no Bank Link' }, { status: 409 })
    ...
    if (action === 'answer_supplier') {
      ...
      // Linha com registro parecido na FOLHA (passagem com deriva, paga pela BR no papel) não entra
      // em regra nem em PESSOAL — CASAR COM AJUSTE decide; senão a mesma passagem vira custo duas vezes.
      let held: any[] = []
      try {
        const [expRowsA, staffA, takenA] = await Promise.all([expensesRows(db, undefined, (q: any) => q.gte('expense_date', since150())), staffNames(db), expenseTaken(db)])
        const expFreeA = expRowsA.filter((r: any) => !takenA.has(String(r.id)))
        held = lines0.filter((l: any) => nearExpenseMatches(l, expFreeA, staffA, rejectedOf(l)).length > 0)
    ...
    if (action === 'reject_twin') {
      ...
      const candList = (Array.isArray(body.cands) ? body.cands.map(String) : []).filter((x: string) => /^[a-z_]+:.+/.test(x))
      if (!bankId2 || (!candList.length && !/^[a-z_]+:.+/.test(cand))) return NextResponse.json({ error: 'bank_id/cand required' }, { status: 400 })
      ...
      const cands = [...new Set([...(Array.isArray(da0.cands) ? da0.cands : []), ...(da0.cand ? [da0.cand] : []), ...(cand ? [cand] : []), ...candList].map(String))].slice(-20)
      ...
      await db.from('data_fixes').insert({ check_key: 'engine-questions', table_name: 'bank_transactions', row_id: bankId2, field: 'doubt_answered', old_value: null, new_value: cand.slice(0, 200), label: ('PERGUNTA · NÃO É ESSE · ' + (cand || candList.join(','))).slice(0, 200) }).then(() => undefined, () => undefined)
      await loadDbAliases(db)
    ...
    if (action === 'restore_log') {
      ...
      let mixedPool: Pool | null = null   // o pool (saídas e entradas livres), lido uma vez e só se houver misto a restaurar (o writeMatch confere o resto)
      for (const l of lines2) {
        const r = latest.get(String(l.id)); if (!r) continue
        try {
          if (r.action === 'MATCH' && r.matched_table && r.matched_id) {
            if (!['purchase_group', 'kit_group', 'expense_group', MIXED_GROUP].includes(r.matched_table)) {
            ...
            // CASAMENTO MISTO (BL 1.6.0): membro não tem índice único — cada um tem de ser candidato livre do pool AGORA (a régua do match_mixed:
            ...
            // MISTO COM SINAL (BL 1.6.1): a régua do match_mixed inteira — tabela de mixable(), 2 a 10 sem repetido, cada membro livre em
            // UM dos dois pools, pelo menos um do lado da linha e o LÍQUIDO (mixedNet: do lado da linha soma, do oposto desconta) batendo
            // com |valor da linha|. Qualquer falha = o diário não vale mais pra esta linha — fica NEW, como alvo sumido.
            if (r.matched_table === MIXED_GROUP) {
              if (!mixedPool) mixedPool = await candidatePool(db)
              ...
              for (const m of members.filter((x: any) => x.table === 'staff_expenses')) {
                ...
                relinked.push(String(m.id))
                pre.push({ t: 'staff_expenses', id: String(m.id), f: 'bank_transaction_id', v: String(l.id), o: null })
              }
            }
            try { await writeMatch(db, l, { table: r.matched_table, id: r.matched_id, members }, { matched_note: r.note || null, match_engine: r.engine || null, match_batch: attributed ? null : (r.batch || null), reviewed_at: attributed || r.matched_table === MIXED_GROUP ? r.at : null }, pre) }
            catch (e) {
              // Só devolve o elo se o misto NÃO pegou com estes membros; releitura que falha não prova nada — nada é revertido às cegas.
              if (relinked.length) {
                const { data: now1, error: nErr } = await db.from('bank_transactions').select('match_status, matched_table, matched_id, matched_members').eq('id', l.id).maybeSingle()
                const want = new Set(members.map((m: any) => m.table + ':' + m.id)), got = mixedMembers(now1).map(m => m.table + ':' + m.id)
                if (!nErr && !(now1 && now1.match_status === 'MATCHED' && now1.matched_table === MIXED_GROUP && got.length === want.size && got.every(k => want.has(k)))) await unlink()
              }
              throw e
            }
            matched++
    ...
    if (action === 'purge_orphan') {
      ...
      if ((await mixedLinesWith(db, table, rowId)).length) return NextResponse.json({ error: 'o lançamento é membro de um casamento MISTO vivo — não é órfão' }, { status: 409 })
