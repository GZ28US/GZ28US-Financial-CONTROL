...
// estimated_seconds = TEMPO PREVISTO da empreita, em segundos. Vira
// invoice_duties.estimated_seconds ao aplicar o pack, e lá fica de par com
// time_seconds (o REALIZADO cronometrado) pra dar PREVISTO × REALIZADO.
type PackDuty = { description: string; priority: string; estimated_seconds: number | null }
...
export default function EditPackPage() {
  ...
  // Se as quotes nascidas deste pack já saem mostrando o PART NUMBER embaixo
  ...
  // One-line DB reference: MAP · OUR COST · % · WEIGHT (USD — this app's only market).
  ...
  function addDuty() {
    ...
    setDuties(sortPackDuties([...duties, { description: withDutyOrder(newDuty.order, newDuty.description), priority: newDuty.priority, estimated_seconds: dutyEstSeconds(newDuty.est) }]))
    setNewDuty({ order: '', description: '', priority: '1', est: '' })
  }
  ...
  function startEditDuty(index: number) {
    ...
    setEditingDuty({ order, description: order ? stripDutyOrder(desc) : desc, priority: duties[index].priority || '1', est: dutyEstHours(duties[index].estimated_seconds) })
  }
  function saveEditDuty() {
    if (editingDutyIndex === null || !editingDuty.description.trim()) return
    const updated = [...duties]
    updated[editingDutyIndex] = { description: withDutyOrder(editingDuty.order, editingDuty.description), priority: editingDuty.priority, estimated_seconds: dutyEstSeconds(editingDuty.est) }
                  ...
                  <div key={d.id} className="flex items-center justify-between gap-4 border-b border-gray-800 py-2">
                    <div className="min-w-0">
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        <p className="font-bold truncate" title={d.alias || d.item}>{d.alias || d.item}</p>
                      </div>
          ...
          <div className="bg-gray-900 border border-gray-700 rounded-2xl p-4 space-y-3">
            ...
            {!locked && (
              <div className="flex gap-3 items-center flex-wrap">
                ...
                <button onClick={() => setShowPartNumbers(!showPartNumbers)} className={`${showPartNumbers ? 'bg-teal-700 hover:bg-teal-600' : 'bg-gray-600 hover:bg-gray-500'} px-5 py-3 rounded-2xl font-bold text-lg`} title="Quotes born from this pack start with the PART NUMBER shown under each item">🔢 {showPartNumbers ? 'HIDE' : 'SHOW'} PART NUMBERS</button>
                <div className="flex items-center gap-2">
        ...
        <div>
          ...
          <div className="bg-gray-900 border border-gray-700 rounded-2xl p-4 space-y-3">
            ...
            {duties.some(d => d.estimated_seconds) && (
              <p className="text-sm font-bold text-gray-200">⏱ ESTIMATED TOTAL: {fmtDutyEst(duties.reduce((t, d) => t + (Number(d.estimated_seconds) || 0), 0))}</p>
            )}
            {!locked && (
              <>
                <div className="flex gap-3 flex-wrap">
