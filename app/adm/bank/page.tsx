          ...
          <div className="flex items-end justify-between gap-6 flex-wrap border-b border-gray-800 pb-6 mb-8">
            <div>
              ...
              {loading ? <p className="text-6xl font-bold tabular-nums leading-none mt-2 text-gray-600">…</p>
                : liveOnes.length > 0 ? <p className={`text-6xl font-bold tabular-nums leading-none mt-2 ${avail < 0 ? 'text-red-400' : 'text-emerald-400'}`} title="disponível: o mesmo número que o app do banco mostra — current menos as pendências">{avail < 0 ? '−' : ''}{usd(avail)}</p>
          ...
          <div key={a.id} className={`rounded-3xl p-5 border ${a.status === 'ACTIVE' ? 'bg-gray-900 border-gray-800' : 'bg-red-950/40 border-red-800'}`}>
            ...
            {(() => { const li = (liveItems || []).find((i) => i.id === a.id); return li && li.live ? (
              <div className="mt-3">
                ...
                <p className={`text-2xl font-bold tabular-nums ${li.live.available < 0 ? 'text-red-400' : 'text-emerald-400'}`} title="disponível — o número do app do banco">{li.live.available < 0 ? '−' : ''}{usd(li.live.available)}</p>
                <p className="text-xs text-gray-500">disponível · current {usd(li.live.current)} · Plaid agora</p>
              </div>
            ) : null })()}
