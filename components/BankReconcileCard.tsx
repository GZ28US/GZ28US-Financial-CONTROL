...
// só-service-key). Nunca recarrega a página: a linha some ao ser decidida.
...
export default function BankReconcileCard({ onCount }: { onCount?: (n: number) => void }) {
  ...
  async function load() {
    setError('')
    try {
      const r = await fetch(`${BASE_PATH}/api/bank/reconcile`, { headers: await authHeaders() })
      const d = await r.json()
      if (!r.ok) throw new Error(d.error || r.statusText)
      setLines(d.lines); setTotalNew(d.total_new); onCount?.(d.total_new)
      const p: Record<string, string> = {}
      for (const l of d.lines as Line[]) if (l.candidates[0]) p[l.id] = l.candidates[0].table + ':' + l.candidates[0].id
      setPick(p)
  ...
  async function act(l: Line, action: string, extra: Record<string, unknown> = {}) {
    setBusy(l.id)
    try {
      const r = await fetch(`${BASE_PATH}/api/bank/reconcile`, { method: 'POST', headers: await authHeaders(), body: JSON.stringify({ action, bank_id: l.id, ...extra }) })
      const d = await r.json()
      if (!r.ok) { alert(d.error || 'Falhou'); return }
      setLines(prev => (prev || []).filter(x => x.id !== l.id))
      setTotalNew(n => { onCount?.(n - 1); return n - 1 })
  ...
  return (
    <div className={`border rounded-2xl overflow-hidden ${totalNew === 0 && lines ? 'border-emerald-900/60' : 'border-gray-700'}`}>
      <button onClick={() => setOpen(o => !o)} className="w-full text-left px-5 py-4 bg-gray-900 hover:bg-gray-800 flex items-center gap-4">
        <span className={`text-2xl font-bold tabular-nums w-14 shrink-0 ${totalNew === 0 && lines ? 'text-emerald-400' : 'text-amber-300'}`}>{lines ? (totalNew === 0 ? '✓' : totalNew) : '…'}</span>
        <span className="flex-1">
          <span className="font-bold block">Banco sem casamento — linhas da Regions sem par no app</span>
          <span className="text-xs text-gray-500">trava: conciliação do DFC com o extrato · {lines ? `${withCand} com candidato automático` : 'carregando…'}</span>
        </span>
      ...
      {open && (
        <div className="px-5 py-4 border-t border-gray-800">
          ...
          {!lines ? <p className="text-gray-500">Carregando o banco…</p> : visible.length === 0 ? <p className="text-emerald-400 font-bold">Nada pendente aqui.</p> : (
