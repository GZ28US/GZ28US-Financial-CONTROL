    ...
    return (
      <div key={l.id} className="py-3 px-2">
        <div className="flex items-baseline gap-3 flex-wrap">
          ...
          {l.near && l.near[0] && !l.pending && <button disabled={dis} onClick={() => { if (confirm('NÃO é essa passagem da folha? A linha guarda a recusa (todas as linhas do candidato) e o AUTO-LINK segue sem ela.')) act(l, 'reject_twin', { cands: l.near![0].ids.map(i => 'staff_expenses:' + i) }) }} className="bg-gray-800 hover:bg-gray-700 border border-gray-600 disabled:opacity-40 px-2 py-0.5 rounded-xl text-[10px] font-bold shrink-0" title="não é essa passagem da folha">NÃO (folha)</button>}
        </div>
